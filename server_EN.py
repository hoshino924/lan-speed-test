    ...
    try:
        ...
        while True:
            ...
            if action == "ping":
                ...
                send_message(conn, {"type": "upload_result", "bytes": received, "seconds": time.perf_counter() - started})
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
        ...
        while not stop_event.is_set():
            try:
                ...
                continue
            threading.Thread(target=tcp_client, args=(conn, address, udp_port), daemon=True).start()
...
def main():
    ...
    udp_thread = threading.Thread(target=udp_server, args=(args.host, args.udp_port, stop_event), daemon=True)
    udp_thread.start()
    try:
