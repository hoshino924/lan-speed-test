...
def main():
    ...
    try:
        ...
        with conn:
            ...
            print(f"TCP latency: average {statistics.mean(samples):.2f} ms, minimum {min(samples):.2f} ms, P95 {percentile(samples, 0.95):.2f} ms, maximum {max(samples):.2f} ms")
            ...
            print(f"Download speed: {mbps(received, download_seconds):.2f} Mbps ({received / 1024 / 1024:.1f} MiB, {download_seconds:.2f} seconds)")
            print(f"Upload speed: {mbps(sent, upload_seconds):.2f} Mbps ({sent / 1024 / 1024:.1f} MiB, {upload_seconds:.2f} seconds)")

            udp_received, udp_rtts = udp_loss_test(args.server, udp_port, token, args.udp_count, args.udp_interval, args.udp_timeout)
            lost = args.udp_count - udp_received
            detail = f", average successful-packet RTT {statistics.mean(udp_rtts):.2f} ms" if udp_rtts else ""
            print(f"UDP packet loss: {lost / args.udp_count * 100:.2f}% (sent {args.udp_count}, received {udp_received}, lost {lost}{detail})")
